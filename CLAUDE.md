...
- `supabase/functions/_shared/zones.ts`, `language.ts`, `vocab.ts`, `dates.ts`, `translate-leaks.ts`, `coaching.ts` -- the post-transcription passes (see Recording Flow). All pure except `translate-leaks` and `coaching`'s moment-detection call; unit-tested in `tests/`.
