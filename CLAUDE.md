...
- `supabase/functions/rename-speaker/`, `create-followup-event/`, `draft-followup-email/`, `account-brief/` -- meeting actions. All five action functions use `_shared/auth.ts` (user JWT scoped by `user_id`, or the service-role bearer) with `verify_jwt = false` in `config.toml`.
