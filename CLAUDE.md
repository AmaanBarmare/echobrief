...
The local Vercel CLI **is** logged into this project (verified 2026-08-30: `vercel whoami` → `khush007`, team Oltaflock AI, and `/v9/projects/.../domains` lists both echobrief domains). An earlier note here said the owning account was separate and the CLI must not be used — that was wrong. `vercel env ls/add/pull` are the fastest way to inspect and set these. Two gotchas: vars marked **Sensitive** come back from `vercel env pull` as the literal `[SENSITIVE]` rather than their value (so you cannot read `SUPABASE_JWT_SECRET` or `SUPABASE_SERVICE_ROLE_KEY` back), and `vercel env add NAME <env>` takes the value on **stdin**, never as a positional argument.
