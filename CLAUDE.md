...
| [`docs/edge-functions.md`](docs/edge-functions.md) | All 44 functions — triggers, auth, request/response shapes |
...
- `supabase/functions/rename-speaker/`, `create-followup-event/`, `draft-followup-email/`, `account-brief/` -- meeting actions. All five action functions (incl. `regenerate-insights`) use `_shared/auth.ts` (user JWT scoped by `user_id`, or the service-role bearer) with `verify_jwt = true` in `config.toml` — the gateway verifies the signature, `authenticate()` reads the `role` claim. Since the 2026-08-31 auth audit this is the default pattern for every user-facing function; **seven** functions keep `verify_jwt = false` — the three signature-verified webhooks, `google-oauth-redirect`, `microsoft-oauth-redirect`, `get-shared-meeting` and `get-google-client-id`. Regenerate that list rather than trusting it: `awk '/^\[functions\./{f=$0} /verify_jwt *= *false/{print f}' supabase/config.toml`.
