...
| [`docs/edge-functions.md`](docs/edge-functions.md) | All 49 functions — triggers, auth, request/response shapes |
...
- `supabase/functions/slack-oauth-start/`, `slack-oauth-redirect/`, `manage-slack/` + `_shared/slack.ts` / `slack-delivery.ts` -- **Slack summary delivery.** Per-user OAuth install; the bot token is sealed into `slack_connections` (one row per user — that row's existence *is* "connected"). Slack was removed on 2026-08-20 because it shipped a single global `SLACK_BOT_TOKEN` for every customer, a pasted raw channel ID, and a Disconnect that never wrote to the DB; the schema and these functions make all three impossible. **Only summary, decisions and action items are posted** — meeting-zone fields only, never the transcript, coaching, facts or attendee emails, because a channel is a room full of people. `deliverToSlack` claims a `slack_deliveries` row **before** posting, so a regeneration or a replayed Sarvam callback cannot re-post a meeting; it never throws. A fatal Slack error (`token_revoked`, `invalid_auth`, …) sets `needs_reconnect`; a channel error clears `channel_id`, turning "silently not posting" into a visible "pick a channel".
...
- **Run the output-quality evals before deploying anything that touches transcription or insights:** `python3 scripts/evals/run_evals.py`. 11 evals (6 deterministic + 5 LLM-judge) over a 9-case dataset. Six of those cases are **negative controls** — one per scorer, each built to break exactly one property and marked `expect: fail`, so a scorer that has gone blind fails the suite instead of passing quietly. Plus the hand-labeled fixture `case_live_f09a4803` (numbers recall, entity spelling, boundary exclusion). Exit code gates the deploy. See [`scripts/evals/EVALS.md`](scripts/evals/EVALS.md) for the harness-vs-evals distinction and how to grow the dataset from prod meetings.
