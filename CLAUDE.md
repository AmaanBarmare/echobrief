...
- `supabase/functions/manage-billing/` -- Dodo Payments actions for the signed-in user (`verify_jwt = true`): `checkout` creates a subscription checkout session for a **plan** the client names (`starter` | `pro` — `SELLABLE_PLANS`); the plan is resolved to a Dodo product server-side by `productForPlan`, which reads the same `DODO_PLAN_PRODUCTS` map `planForProfile` uses in reverse, so what a customer pays for and what they are entitled to cannot drift. `portal` opens the Dodo customer portal.
...
- `supabase/functions/_shared/entitlements.ts` -- **plan limits and the gate that enforces them.** `PLANS` (free/starter/pro/teams: meeting count, included hours, overage ceiling, per-meeting seconds, retention days), `planForProfile()` (plan_override → Dodo status/product → free), `checkRecordingAllowed()` and the `usage_events` writers. **Every path that creates a Recall bot must call `checkRecordingAllowed` first** — `start-recall-recording` returns 402, `auto-join-meetings` skips the event. The per-meeting ceiling is enforced by Recall itself via `automatic_leave.in_call_recording_timeout`, not by us. The numbers are mirrored in `src/lib/plans.ts` (usage meter) and `src/components/landing/Pricing.tsx` (the page customers read) — **change all three together or none.** Fails OPEN on a usage-read error: a DB blip must not stop every customer recording.
...
- `prune-content` — **daily 03:45 UTC**. Plan-aware retention (14/30/90 days). Deliberately clear of the other two prune jobs so the three never contend for the same IO tick.
...
- `DODO_PLAN_PRODUCTS` -- `{"<dodo product id>": "starter" | "pro"}`; the single map behind both entitlement resolution and checkout product selection
...
- `DODO_ENVIRONMENT` -- `test_mode` (default) or `live_mode`; picks the API base URL. **Live since 2026-09-01**: EchoBrief Starter ₹799/mo and EchoBrief Pro ₹1999/mo are real Dodo products, and the live webhook endpoint is subscribed to the nine `subscription.*` events `mapEventToStatus` handles.
