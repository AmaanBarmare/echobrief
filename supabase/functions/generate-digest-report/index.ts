...
function generateDigestHTML(
  ...
  const meetingsByDate = meetings.reduce((acc: any, m: any) => {
    const date = formatISTDate(m.start_time, { weekday: 'long', month: 'short', day: 'numeric' })
...
<head>
  ...
  <style>
    ...
    .stat-box { background: #F8E7DF; padding: 20px; border-radius: 6px; border-left: 4px solid #D93F0B; }
    .stat-number { font-size: 28px; font-weight: bold; color: #D93F0B; }
    .stat-label { font-size: 12px; color: #827873; margin-top: 5px; }
    ...
    .meeting-date { font-size: 13px; font-weight: 600; color: #D93F0B; margin-bottom: 8px; }
    .meeting-item { background: #FAF4EF; padding: 12px; border-radius: 4px; margin-bottom: 8px; }
    .meeting-title { font-weight: 600; color: #190F0B; }
    .meeting-summary { font-size: 13px; color: #666; margin-top: 4px; }
    .action-item { background: #FAF4EF; padding: 12px; border-radius: 4px; margin-bottom: 8px; border-left: 3px solid #479C4D; }
    .action-task { font-weight: 500; color: #190F0B; }
...
<body>
  <div class="container">
    ...
    <div class="footer">
      ...
      <p style="margin-top: 10px;"><a href="https://echobrief-ten.vercel.app" style="color: #D93F0B; text-decoration: none;">View full reports →</a></p>
    </div>
  </div>
</body>
</html>
...
serve(async (req) => {
  ...
  try {
    ...
    // Generate HTML
    const startFormatted = formatISTDate(start, { month: 'short', day: 'numeric', year: 'numeric' })
    const endFormatted = formatISTDate(end, { month: 'short', day: 'numeric', year: 'numeric' })
